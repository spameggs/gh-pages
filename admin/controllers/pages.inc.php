...
if ($_GET['q'] == 'ext') {
    ...
    if ($_GET['action'] == 'add' || $_GET['action'] == 'edit') {
        ...
        $all_pages = $rlDb->fetch(array(
            'ID',
            'Key'
        ), array(
            'Status' => 'active'
        ), "AND `Key` <> 'home' ORDER BY `Key`", null, 'pages');
        $all_pages = $rlLang->replaceLangKeys($all_pages, 'pages', array(
            'name'
        ));
        $rlSmarty->assign_by_ref('all_pages', $all_pages);
        if ($_GET['action'] == 'edit' && !$_POST['fromPost']) {
            $key                = $rlValid->xSql($_GET['page']);
            $info               = $rlDb->fetch('*', array(
                'Key' => $key
            ), "AND `Status` <> 'trash'", null, 'pages', 'row');
            $_POST['key']       = $info['Key'];
            ...
            if (!empty($errors)) {
                ...
                if ($_GET['action'] == 'add') {
                    ...
                    foreach ($allLangs as $key => $value) {
                        ...
                        if ($f_page_type == 'static') {
                            $exist_content = $rlDb->fetch(array(
                                'ID'
                            ), array(
                                'Key' => 'pages+content+' . $f_key,
                                'Code' => $allLangs[$key]['Code']
