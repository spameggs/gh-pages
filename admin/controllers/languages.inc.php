...
if ($_GET['q'] == 'ext_list') {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'search') {
        ...
        $lang_data = $rlDb->fetch(array(
            ...
            'Code' => $langCode,
            'Status' => 'active'
        ), "AND `Module` <> 'email_tpl' AND `Key` NOT LIKE 'data_formats+name+%' ORDER BY `{$sort}` {$sortDir}", array(
            $start,
            $limit
        ));
        $rlDb->resetTable();
    ...
    if ($_GET['action'] == 'update') {
        ...
        if ($_GET['compare_mode'] == "phrases") {
            set_time_limit(0);
            $rlDb->setTable('lang_keys');
            $phrases_1_tmp = $rlDb->fetch('*', array(
                'Code' => $lang_1
            ), "AND `Status` <> 'trash' ORDER BY `Key`");
            foreach ($phrases_1_tmp as $pK => $pV) {
                $phrases_1[$phrases_1_tmp[$pK]['Key']] = $phrases_1_tmp[$pK];
            }
            unset($phrases_1_tmp);
            $phrases_2_tmp = $rlDb->fetch('*', array(
                'Code' => $lang_2
            ), "AND `Status` <> 'trash' ORDER BY `Key`");
    ...
    if ($_GET['action'] == 'edit') {
        ...
        if ($_GET['action'] == 'edit' && !$_POST['fromPost']) {
            ...
            $l_name               = $rlDb->fetch(array(
                'Code',
                'Value'
            ), array(
                'Key' => 'languages+name+' . $language['Key']
            ), "AND `Status` <> 'trash'", 1, 'lang_keys', 'row');
        ...
        if (!empty($errors)) {
            ...
            if ($_POST['compare_mode'] == "phrases") {
                ...
                if (empty($adapted_compare_1) && empty($adapted_compare_2)) {
                    ...
                    $compare_lang1         = array(
                        'diff' => count($adapted_compare_1),
                        'Code' => $lang_1
                    );
                    $compare_lang2         = array(
                        'diff' => count($adapted_compare_2),
                        'Code' => $lang_2
                    );
