...
if ($_GET['q'] == 'ext') {
    ...
    if ($_GET['action'] == 'remote_activation' && $_GET['id'] && $_GET['hash']) {
        ...
        $accounts_list = $rlDb->fetch(array(
            'ID',
            'Username'
        ), array(
            'Status' => 'active'
                        ...
                        if ($errors) {
                            ...
                            if ($rlListings->create($info, $data, $category_fields)) {
                                ...
                                if ($_SESSION['listings_redirect_mode']) {
                                    $aUrl = array(
                                        "controller" => "browse",
                                        "id" => $_SESSION['listings_redirect_ID']
                                    );
                                } else {
                                    $aUrl = array(
                    ...
                    if (empty($form)) {
                        ...
                        if (!isset($_POST['fromPost'])) {
                            if (strpos($listing['Crossed'], ',') !== false && !empty($listing['Crossed'])) {
                                ...
                                $_POST['crossed_categories'] = array(
                                    $listing['Crossed']
                                );
                            } else {
                                    ...
                                    switch ($listing_fields[$key]['Type']) {
                                        ...
                                        default:
                                            if (in_array($value['Type'], array(
                                                'text',
                                                'textarea'
                                            )) && $listing_fields[$key]['Multilingual'] && count($GLOBALS['languages']) > 1) {
