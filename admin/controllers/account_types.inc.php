...
if ($_GET['q'] == 'ext') {
    ...
    $data = $rlLang->replaceLangKeys($data, 'account_types', array(
        'name'
    ), RL_LANG_CODE, 'admin');
    ...
    if ($_GET['action']) {
        ...
        $rlXajax->registerFunction(array(
            'preAccountTypeDelete',
            $rlAdmin,
            'ajaxPreAccountTypeDelete'
        ));
        $rlXajax->registerFunction(array(
            'deleteAccountType',
            $rlAdmin,
            'ajaxDeleteAccountType'
        ));
    ...
    if ($_GET['action'] == 'add' || $_GET['action'] == 'edit') {
        ...
        if ($_GET['action'] == 'edit') {
            ...
            $item_info = $rlDb->fetch('*', array(
                'Key' => $i_key
            ), "AND `Status` <> 'trash'", null, 'account_types', 'row');
        }
        ...
        if (!$type_key || !$form_type) {
            ...
            $type_info = $rlLang->replaceLangKeys($type_info, 'account_types', array(
                'name'
            ), RL_LANG_CODE, 'admin');
            ...
            $avail_fields = $rlDb->fetch(array(
                'Group_ID',
                'Field_ID'
            ), array(
                'Category_ID' => $type_info['ID']
            ), null, null, 'account_search_relations');
            ...
            $fields = $rlDb->fetch(array(
                ...
                'Type',
                'Status'
            ), null, "WHERE `Status` <> 'trash' {$add_cond}", null, 'account_fields');
