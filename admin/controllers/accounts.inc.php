    ...
    if ($_GET['action'] == 'remote_activation' && $_GET['id'] && $_GET['hash']) {
        ...
        if ($_GET['action'] == 'add' || $_GET['action'] == 'edit') {
            ...
            if ($_GET['action'] == 'edit' && !$_POST['fromPost']) {
                ...
                $account_fields = $rlLang->replaceLangKeys($account_fields, 'account_fields', array(
                    'name',
                    'description'
                ));
            ...
            if (isset($_POST['form_submit'])) {
                ...
                if ($_GET['action'] == 'add') {
                    $account_exist = $rlDb->fetch(array(
                        'Username'
                    ), array(
                ...
                if ($_GET['action'] == 'add') {
                    $email_exist = $rlDb->fetch('Mail', array(
                        'Mail' => $profile_data['mail']
                    ), null, null, 'accounts', 'row');
                } else {
                    $email_exist = $rlDb->fetch('Mail', array(
                        'Mail' => $profile_data['mail']
                    ), "AND `Mail` <> '{$account_info['Mail']}'", null, 'accounts', 'row');
                }
        ...
        $rlXajax->registerFunction(array(
            'getAccountFields',
            $rlAdmin,
            'ajaxGetAccountFields'
        ));
        $rlXajax->registerFunction(array(
            'updateAccountFields',
            $rlAdmin,
            'ajaxUpdateAccountFields'
        ));
    }
