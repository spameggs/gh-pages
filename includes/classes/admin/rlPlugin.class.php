...
class rlPlugin extends reefless
{
    ...
    function ajaxInstall($key = false, $remote_mode = false)
    {
        ...
        if (is_readable($path_to_install)) {
            ...
            $rlParser->xml_set_element_handler(array(
                &$this,
                "startElement"
            ), array(
                &$this,
            ...
            $rlParser->xml_set_character_data_handler(array(
                &$this,
                "charData"
            ));
            $rlParser->xml_set_comment_handler(array(
                &$this,
                "commentElement"
            ));
            ...
            if ($GLOBALS['rlActions']->insertOne($plugin, 'plugins')) {
                ...
                if (!empty($blocks)) {
                    foreach ($blocks as $key => $value) {
                        ...
                        if (in_array(strtolower($blocks[$key]['Type']), array(
                            'html',
                            'php',
                            'smarty'
                        ))) {
                ...
                if (!empty($pages)) {
                    foreach ($pages as $key => $value) {
                        ...
                        if (in_array($pages[$key]['Page_type'], array(
                            'system',
                            'static',
                            'external'
                        ))) {
                ...
                if (version_compare($update_item['Version'], $current_version) > 0) {
                    ...
                    if (!empty($update_item['Files'])) {
                        ...
                        foreach ($update_files as $update_file) {
                            $file_to_copy = str_replace(array(
                                '\\',
                                '/'
                            ), array(
                                RL_DS,
                    ...
                    if ($success) {
                        ...
                        if (!empty($blocks)) {
                            foreach ($blocks as $key => $value) {
                                if (version_compare($value['Version'], $update_item['Version']) == 0) {
                                    if (in_array(strtolower($value['Type']), array(
                                        'html',
                                        'php',
                                        'smarty'
                                    ))) {
                        ...
                        if (!empty($pages)) {
                            foreach ($pages as $key => $value) {
                                if (in_array($value['Page_type'], array(
                                    'system',
                                    'static',
                                    'external'
                                ))) {
