...
class rlAjaxLang extends reefless
{
    ...
    function ajax_addLanguage($data)
    {
        ...
        $lang_name = $lang_key = $this->rlValid->xSql(str_replace(array(
            '"',
            "'"
        ), array(
            '',
        ...
        $lang_key = strtolower(str_replace(array(
            '"',
            "'"
        ), array(
            '',
        ...
        if (!empty($error)) {
            ...
            $source_phrases = $this->fetch('*', array(
                'Code' => $source_code
            ));
    ...
    function ajaxDeleteLang($id)
    {
        ...
        if (count($GLOBALS['languages']) == 2) {
            $multilang_fields_listings = $this->fetch(array(
                'Key'
            ), array(
                'Multilingual' => '1'
            ...
            $multilang_fields_accounts = $this->fetch(array(
                'Key'
            ), array(
                'Multilingual' => '1'
            ), null, null, 'account_fields');
    ...
    function exportLanguage($id = false)
    {
        ...
        if ($phrases) {
            ...
            $content   = "-- Escort Agency Software" . PHP_EOL . "-- Direction: UNDEFINED" . PHP_EOL . "-- Export date: " . date('Y.m.d') . PHP_EOL . "-- version: {$config['rl_version']}" . PHP_EOL . "-- Language SQL Dump: {$lang_name}" . PHP_EOL . "-- nulled by mrx02de" . PHP_EOL . PHP_EOL . "INSERT INTO `{prefix}languages` (`Code`, `Key`, `Status`, `Date_format`, `Direction`) VALUES ('{$info['Code']}', '{$info['Key']}', 'active', '{$info['Date_format']}', '{$info['Direction']}');" . PHP_EOL . PHP_EOL;
            $content .= $insert;
            foreach ($phrases as $key => $value) {
