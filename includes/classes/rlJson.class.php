...
class rlJson
{
    ...
    function encode($var)
    {
        ...
        switch (gettype($var)) {
            ...
            case 'object':
                ...
                return '{' . join(',', $properties) . '}';
            default:
                return ($this->use & rlJson_SUPPRESS_ERRORS) ? 'null' : new rlJson_Error(gettype($var) . " can not be encoded as JSON string");
        }
    }
    function name_value($name, $value)
    {
        $encoded_value = $this->encode($value);
        if (rlJson::isError($encoded_value)) {
            ...
            default:
                ...
                if (is_numeric($str)) {
                    ...
                    for ($c = 0; $c <= $strlen_chrs; ++$c) {
                        $top             = end($stk);
                        $substr_chrs_c_2 = substr($chrs, $c, 2);
                        if (($c == $strlen_chrs) || (($chrs{$c} == ',') && ($top['what'] == rlJson_SLICE))) {
                            ...
                            } elseif (reset($stk) == rlJson_IN_OBJ) {
                                $parts = array();
                        ...
                        } elseif ((($chrs{$c} == '"') || ($chrs{$c} == "'")) && ($top['what'] != rlJson_IN_STR)) {
                            array_push($stk, array(
                                'what' => rlJson_IN_STR,
                                'where' => $c,
                                'delim' => $chrs{$c}
                            ));
                        } elseif (($chrs{$c} == $top['delim']) && ($top['what'] == rlJson_IN_STR) && ((strlen(substr($chrs, 0, $c)) - strlen(rtrim(substr($chrs, 0, $c), '\\'))) % 2 != 1)) {
                            array_pop($stk);
                        ...
                        } elseif (($chrs{$c} == ']') && ($top['what'] == rlJson_IN_ARR)) {
                            array_pop($stk);
                        } elseif (($chrs{$c} == '{') && in_array($top['what'], array(
                            rlJson_SLICE,
                            rlJson_IN_ARR,
                        ...
                        } elseif (($chrs{$c} == '}') && ($top['what'] == rlJson_IN_OBJ)) {
                            array_pop($stk);
                            ...
                            $c++;
                        } elseif (($substr_chrs_c_2 == '*/') && ($top['what'] == rlJson_IN_CMT)) {
                            array_pop($stk);
                            $c++;
    ...
    function isError($data, $code = null)
    {
        ...
        } elseif (is_object($data) && (get_class($data) == 'rlJson_error' || is_subclass_of($data, 'rlJson_error'))) {
            return true;
        }
        return false;
    }
...
if (class_exists('PEAR_Error')) {
    ...
    {
        function rlJson_Error($message = 'unknown error', $code = null, $mode = null, $options = null, $userinfo = null)
        {
