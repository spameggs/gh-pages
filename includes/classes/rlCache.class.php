...
class rlCache extends reefless
{
    ...
    function get($key = false, $id = false, $type = false)
    {
        ...
        if ($type && !$out && in_array($key, array(
    ...
    function updateCategoriesByParent()
    {
        ...
        if ($rlListingTypes->types) {
            foreach ($rlListingTypes->types as $key => $value) {
                $out[$value['Key']] = $this->getChildCat(array(
                    0
                ), $value);
            }
            if ($out) {
                ...
                foreach ($tmp_categories as $cKey => $cValue) {
                    ...
                    if ($type['Cat_show_subcats']) {
                        ...
                        $subCategories  = $this->fetch(array(
                            'ID',
                            'Path`, CONCAT("categories+name+", `Key`) AS `pName`, CONCAT("categories+title+", `Key`) AS `pTitle',
                            'Key'
                        ), array(
                            'Status' => 'active',
                            'Parent_ID' => $cValue['ID']
                        ), "ORDER BY `Position`", null, 'categories');
                        $this->calcRows = false;
    ...
    function updateListingStatistics($listing_type = false)
    {
        ...
        $types = $listing_type && $rlListingTypes->types[$listing_type] ? array(
            $rlListingTypes->types[$listing_type]
