...
class rlZip
{
    ...
    public $pack_dirs = array();
    function get_files_from_folder($directory, $put_into, $save_mode = false)
    {
        global $license_tpl, $license_key, $domain, $system_template, $account_id;
        if ($handle = opendir($directory)) {
            while (false !== ($file = readdir($handle))) {
                if (is_file($directory . $file)) {
                    ...
                    $this->get_files_from_folder($directory . $file . '/', $put_into . $file . '/', $save_mode);
                }
            }
        }
        closedir($handle);
