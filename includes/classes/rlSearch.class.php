...
class rlSearch extends reefless
{
    ...
    function getFields($key = false, $listing_type_key = false, $tab_form = false)
    {
        ...
        if ($config['cache']) {
            ...
            $this->fields = $this->rlLang->replaceLangKeys($fields, 'listing_fields', array(
                'name',
                'default'
            ));
            if ($tab_form && $arrange_field) {
                $a_field = $this->fetch(array(
                    'ID',
                    'Key',
                    'Type'
                ), array(
        ...
        foreach ($relations as $key => $value) {
            ...
            $sql .= "ORDER BY `Order`";
            $fields = $this->getAll($sql);
            $fields = $this->rlLang->replaceLangKeys($fields, 'listing_fields', array(
                'name',
                'default'
            ));
    ...
    function search($data = false, $type = false, $start = 0, $limit = false)
    {
        ...
        $data['sort_type'] = in_array($data['sort_type'], array(
            'asc',
            'desc'
        )) ? $data['sort_type'] : 'asc';
        if ($custom_order) {
    ...
    function keywordSearch($query = false, $mode = 2, $type = false)
    {
        ...
        $fields = $this->fetch(array(
            ...
            'Status' => 'active'
        ), "AND FIND_IN_SET(`Type`, 'bool,file,accept,image') = 0 AND `Key` != 'keyword_search' AND `Key` <> 'search_account' ");
        if (!$fields)
            return;
        ...
        if ($indirect_fields) {
            ...
            if ($indirect_keys) {
                ...
                foreach ($indirect_keys as $ind_key) {
                    ...
                    if ($ind_key['Df']) {
                        if (!$df_fields) {
                            $df_fields_tmp = $this->fetch(array(
                                'Key'
                            ), array(
                                'Status' => 'active'
                            ), "AND `Type` IN ('select', 'radio', 'mixed', 'checkbox') AND `Condition` <> ''", null, 'listing_fields');
