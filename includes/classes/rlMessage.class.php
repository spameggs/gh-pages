...
class rlMessage extends reefless
{
    ...
    function ajaxSendMessage($res_id = false, $message = false, $admin = false)
    {
        ...
        if (defined('IS_LOGIN') || defined('REALM')) {
            ...
            $GLOBALS['rlHook']->load('rlMessagesAjaxSendMessage', $res_id, $message, $admin);
    ...
    function getContacts()
    {
        ...
        $GLOBALS['rlHook']->load('rlMessagesGetContactsSql', $sql);
    ...
    function getMessages($id = false, $no_update = false)
    {
        ...
        $GLOBALS['rlHook']->load('rlMessagesGetMessagesSql', $sql);
        $sql .= "ORDER BY `ID` ASC";
        $messages = $this->getAll($sql);
        foreach ($messages as $key => $value) {
    ...
    function ajaxContactOwner($name = false, $email = false, $phone = false, $message = false, $code = false, $listing_id = false)
    {
        ...
        $GLOBALS['rlHook']->load('rlMessagesAjaxContactOwnerValidate', $name, $email, $phone, $message, $listing_id);
        if (empty($message)) {
        ...
        if ($errors) {
            ...
            $GLOBALS['rlHook']->load('rlMessagesAjaxContactOwnerSend', $name, $email, $phone, $message, $listing_id);
            if (defined('IS_LOGIN')) {
                $insert = array(
                    'From' => $account_info['ID'],
                    'To' => $info['Account_ID'],
    ...
    function ajaxRemoveMsg($ids = false, $contact_id = false)
    {
        ...
        $ids = explode(',', $ids);
        if (!empty($ids[0])) {
            $GLOBALS['rlHook']->load('rlMessagesAjaxRemoveMsg', $ids, $contact_id);
            foreach ($ids as $id) {
                $res = $this->fetch(array(
                    ...
                    'To',
                    'Remove'
                ), null, "WHERE ((`From` = '{$account_info['ID']}' AND `To` = '{$contact_id}') OR (`From` = '{$contact_id}' AND `To` = '{$account_info['ID']}')) AND `ID` = '{$id}'", 1, 'messages', 'row');
                if ($res) {
                    $request  = $account_info['ID'] == $res['From'] ? 'from' : 'to';
                    $update[] = array(
                        'fields' => array(
                            'Remove' => $res['Remove'] . ',' . $request
                        ),
                        'where' => array(
                            'ID' => $id
                        )
    ...
    function ajaxRemoveContacts($ids = false)
    {
        ...
        if (!empty($ids[0])) {
            $this->loadClass('Actions');
            $GLOBALS['rlHook']->load('rlMessagesAjaxRemoveContacts', $ids);
            foreach ($ids as $contact_id) {
                ...
                foreach ($messages as $key => $value) {
                    if ($value['From'] == $value['To']) {
                        ...
                        if (in_array($account_info['ID'], array(
                            $value['From'],
                            $value['To']
                        ))) {
