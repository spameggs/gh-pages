...
class rlListings extends reefless
{
            ...
            foreach ($fields as $key => $value) {
                ...
                if (isset($data[$fields[$key]['Key']])) {
                    ...
                    switch ($fields[$key]['Type']) {
                        ...
                        case 'price':
                            if (!empty($data[$fk]['value'])) {
								if ($GLOBALS['config']['price_decimal'] == ","){
									$fpdecimal = ".";
								} else {
									$fpdecimal = ",";
								};
                                $data[$fk]['value'] = str_replace(array(
                                    $fpdecimal,
                                    "'"
                                ), '', $data[$fk]['value']);
    ...
    function getMyListings($type = false, $order = 'ID', $order_type = 'asc', $start = 0, $limit = false)
    {
        ...
        $GLOBALS['rlHook']->load('myListingsSqlFields', $sql);
        ...
        $GLOBALS['rlHook']->load('myListingsSqlJoin', $sql);
        ...
        $sql .= "LIMIT {$start}, {$limit}";
        $GLOBALS['rlHook']->load('myListingsSql', $sql);
        $listings = $this->getAll($sql);
        if (empty($listings)) {
            return false;
        }
    ...
    function getListingDetails($id, &$listing, $listing_type = false)
    {
        ...
        foreach ($form as $gKey => $group) {
            if ($group['Fields']) {
                ...
                $form[$gKey]['Fields'] = $this->rlLang->replaceLangKeys($form[$gKey]['Fields'], 'listing_fields', array(
                    'name'
                ));
            }
        }
    ...
    function getFormFields($id = false, $table = 'short_forms', $type = false)
    {
        ...
        if ($config['cache']) {
            ...
            $fields = $GLOBALS['rlLang']->replaceLangKeys($fields, 'listing_fields', array(
                'name',
                'default'
            ));
            return $fields;
    ...
    function ajaxDeletePhoto($listing_id, $photo_id)
    {
        ...
        if ($this->query($sql)) {
            ...
            $photos = $this->fetch('*', array(
                'Listing_ID' => $listing_id
            ), "ORDER BY `ID`", null, 'listing_photos');
    ...
    function ajaxTellFriend($friend_name, $friend_email, $your_name = null, $your_email = null, $message, $security_code = false, $listing_id)
    {
        ...
        if (!empty($errors)) {
            ...
            $link             = $GLOBALS['config']['mod_rewrite'] ? RL_URL_HOME . $page_info['Path'] . '/' . $listing_data['Path'] . '/' . $GLOBALS['rlSmarty']->str2path(array(
                'string' => $listing_title
            )) . '-l' . $listing_data['ID'] . '.html' : RL_URL_HOME . 'index.php?page=' . $page_info['Path'] . '&amp;id=' . $listing_data['ID'];
                ...
                if ($config['trash']) {
                    if ($info['Category_ID']) {
                        $GLOBALS['rlActions']->delete(array(
                            'ID' => $info['ID']
                        ), 'listings', $info['ID'], 1, $info['ID']);
                        if ($this->isActive($info['ID'])) {
    ...
    function getFeatured($type = false, $limit = 10, $field = false, $value = false)
    {
        ...
        $sql .= "LEFT JOIN `" . RL_DBPREFIX . "accounts` AS `T7` ON `T1`.`Account_ID` = `T7`.`ID` ";
        $sql .= "WHERE ( ";
        $sql .= " TIMESTAMPDIFF(HOUR, `T1`.`Featured_date`, NOW()) <= `T2`.`Listing_period` * 24 ";
        $sql .= " OR `T2`.`Listing_period` = 0 ";
        $sql .= ") ";
        $sql .= "AND ( ";
        $sql .= " TIMESTAMPDIFF(HOUR, `T1`.`Pay_date`, NOW()) <= `T3`.`Listing_period` * 24 ";
    ...
    function upgradeListing($listing_id, $plan_id, $account_id, $txn_id, $gateway, $total)
    {
        ...
        if (!empty($listing_info) && !empty($plan_info)) {
            ...
            $replace          = array(
                ...
                $gateway,
                $txn_id,
                $GLOBALS['lang'][$plan_info['Type'] . '_plan'],
                $plan_info['Price'],
            ...
            case 'local':
                ...
                if (!empty($video_tmp['tmp_name']) && !array_key_exists($video_file_ext, $l_player_file_types)) {
                    $errors[] = str_replace(array(
                        ...
                        '<span class="field_error">"' . $lang['file'] . '"</span>',
                        '<span class="field_error">"' . $video_file_ext . '"</span>'
                ...
                if (!$rlValid->isImage($preview_file_ext) && !empty($preview_tmp['tmp_name'])) {
                    $errors[] = str_replace(array(
                        ...
                        '<span class="field_error">"' . $lang['preview_image'] . '"</span>',
                        '<span class="field_error">"' . $preview_file_ext . '"</span>'
                    ), $lang['notice_bad_file_ext']);
                }
                if (empty($errors)) {
                    ...
                    if (move_uploaded_file($video_tmp['tmp_name'], $file_location)) {
                        if (move_uploaded_file($preview_tmp['tmp_name'], $thumbnail_location)) {
                            ...
                            $rlCrop->flushImages();
                            $rlResize->resize($thumbnail_location, $thumbnail_location, 'C', array(
                                120,
                                90
                            ), true, false);
                        }
    ...
    function buildFeaturedBoxes($listing_type_key = false)
    {
        ...
        if ($rlMobile->isMobile && $config['mobile_version_module']) {
            ...
            $mobile_featured_blocks = $this->fetch(array(
                'Key'
            ), null, "WHERE `Key` RLIKE 'ltfb_(.*)[^_box0-9]$'");
            $this->resetTable();
    ...
    function replaceMetaFields($category_id = false, $listing_data = false, $type = 'description')
    {
        ...
        if ($tpl = $GLOBALS['lang']['categories+listing_meta_' . $type . '+' . $cat_info['Key']]) {
            ...
            $fields_info_tmp = $this->fetch("*", array(
                'Status' => 'active'
            ), "AND FIND_IN_SET(`Key`, '" . implode(",", $fields[1]) . "')", null, 'listing_fields');
