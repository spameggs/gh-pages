...
if ($get_step || $_GET['id'] || $_REQUEST['tmp_id']) {
    if ($_REQUEST['tmp_id'] || $_SESSION['add_listing']['tmp_id']) {
        ...
        $tmp_cat                           = $rlDb->fetch(array(
            'Name',
            'Parent_ID'
        ), array(
            'ID' => $tmp_id
        ), null, 1, 'tmp_categories', 'row');
    ...
    if (!$errors) {
        ...
        if ($get_step) {
            ...
            if (!$plan_id && !in_array($cur_step, array(
                'category',
                'plan'
            ))) {
                $url = SEO_BASE;
                                    ...
                                    switch ($category_fields[$key]['Type']) {
                                        ...
                                        default:
                                            if (in_array($value['Type'], array(
                                                'text',
                                                'textarea'
                                            )) && $category_fields[$key]['Multilingual'] && count($GLOBALS['languages']) > 1) {
