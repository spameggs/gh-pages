...
//									nextEffect: 'fade',
			...
			<ul class="statistics">
				...
				{if !$single_category_mode}<li><span class="name">{$lang.category}:</span> <a title="{$lang[$listing_data.Category_pName]}" href="{$rlBase}{if $config.mod_rewrite}{$pages[$listing_type.Page_key]}/{$listing_data.Path}{if $listing_type.Cat_postfix}.html{else}/{/if}{else}?page={$pages[$listing_type.Page_key]}&amp;category={$listing_data.Category_ID}{/if}">{$lang[$listing_data.Category_pName]}</a></li>{/if}
				...
				{if $seller_info.Type == 'personal' && $config.messages_module && ($isLogin || (!$isLogin && $config.messages_allow_free))}
					<li style="padding: 20px 0 0 0;">
						<input id="contact_owner" type="button" value="{$lang.contact_owner}" />
					</li>
				{/if}
		...
		<td valign="top" class="details">
			<div class="highlight">
				...
				{if $config.map_module && $location}
					<div id="map" style="margin: 5px 0 0;width: {if empty($config.map_width)}100%{else}{$config.map_width}px{/if}; height: {if empty($config.map_height)}300px{else}{$config.map_height}px{/if}"></div>
...
<!-- video tab -->
{if !empty($videos)}
<div id="area_video" class="tab_area hide padding">
...
<!-- tell a friend tab -->
<div id="area_tell" class="tab_area hide padding">
	<div class="highlight">
		<table class="submit">
		<tr>
