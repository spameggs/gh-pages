...
<div class="highlight clear"> {if !$cur_step} 
  ...
  <div class="area_plan step_area hide">
    ...
    <form method="post" action="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/{$category.Path}/{$steps.$cur_step.path}.html{else}?page={$pageInfo.Path}&amp;id={$category.ID}&amp;step={$steps.$cur_step.path}{/if}">
        ...
        <tr {if $plan.ID == $smarty.post.plan}class="active"{/if}> {assign var='item_disabled' value=false}
          ...
          <td class="info"><table class="sTable{if $plan.Limit > 0 && $plan.Using == 0 && $plan.Using != ''} o60{/if}">
              <tr>
                ...
                <td class="ralign"><div class="status" title="{$lang.package_purchased}">{$lang.available}</div></td>
                {/if} </tr>
            </table>
      ...
      <table class="submit">
        <tr>
          <td class="name button"><a href="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}.html?edit{else}?page={$pageInfo.Path}&amp;edit{/if}" class="dark_12">{if $smarty.const.RL_LANG_DIR == 'ltr'}&larr;{else}&rarr;{/if} {$lang.perv_step}</a></td>
  ...
  <div class="area_form step_area hide">
    ...
    <form enctype="multipart/form-data" method="post" action="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/{$category.Path}/{$steps.$cur_step.path}.html{else}?page={$pageInfo.Path}&amp;id={$category.ID}&amp;step={$steps.$cur_step.path}{/if}">
      ...
      <div class="auth">
        ...
        <!-- print sections/categories tree -->
        <div id="crossed_tree" class="tree{if $smarty.post.crossed_done} hide{/if}"> {foreach from=$sections item='section'}
          {if !$section.Admin_only}
          ...
          {/foreach} </div>
        <!-- print sections/categories tree end -->
        <ul class="hide" id="crossed_selected">
          <li class="first dark"><b>{$lang.selected_crossed_categories}</b></li>
        </ul>
      ...
      <div class="auth">
        <table>
          <tr>
            <td class="side"><div class="lalign">
      ...
      <table class="submit">
        ...
        <tr>
          ...
          <td class="field{if $config.security_img_add_listing} button{/if}"><span class="arrow">
      ...
      {/if} <a href="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/{$category.Path}/{$steps.plan.path}.html{else}?page={$pageInfo.Path}&amp;id={$category.ID}&amp;step={$steps.plan.path}{/if}" class="dark_12">{if $smarty.const.RL_LANG_DIR == 'ltr'}&larr;{else}&rarr;{/if} {$lang.perv_step}</a> <span class="arrow">
      <input type="submit" value="{$lang.next_step}" id="form_submit" />
      <label for="form_submit" class="right">&nbsp;</label>
      </span> {/if}
    </form>
  ...
  <div class="area_photo step_area hide">
    ...
    <div id="crop_block" class="hide">
      ...
      {include file='blocks'|cat:$smarty.const.RL_DS|cat:'fieldset_footer.tpl'} </div>
    <!-- file crop end --> 
  </div>
  {/if} 
  <!-- add photo end --> 
  ...
  <div id="area_video" class="area_video step_area hide">
    ...
    <ul class="thumbnails inline">
      ...
      <li id="video_{$video.ID}" class="active"> {if $video.Type == 'local'} <img class="item" src="{$smarty.const.RL_FILES_URL}{$video.Preview}" alt="" /> {else} <img class="item" src="{$l_youtube_thumbnail|replace:$replace:$video.Preview}" alt="" /> {/if} <img src="{$rlTplBase}img/blank.gif" class="delete" alt="{$lang.delete}" title="{$lang.delete}" /> 
  ...
  <div class="area_done step_area hide">
    ...
    <span class="dark"> {if $account_info.Type == 'personal'}
    {if $config.mod_rewrite}
    {assign var='replace_link' value=$rlBase|cat:$pages[$account_info.my_profile_page_key]|cat:'.html'}
    {else}
    {assign var='replace_link' value=$rlBase|cat:'?page='|cat:$pages[$account_info.my_profile_page_key]|cat:'.html'}
    {/if}
    {assign var='replace' value='<a href="'|cat:$replace_link|cat:'">$1</a>'}
    {$lang.escort_done_message|regex_replace:'/\[(.*)\]/':$replace}
    {else}
