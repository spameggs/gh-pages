...
<div class="highlight">
		...
		<ul class="account_menu">
			{foreach from=$account_menu item='mItem'}
				{if $mItem.Key == 'my_messages' && !$config.messages_module}{else}
					<li>
						<a title="{if $account_info.Type == 'personal' && ($mItem.Key == 'add_listing' || $mItem.Controller == 'my_listings')}{$lang.escort_my_profile}{else}{$mItem.title}{/if}" href="{$rlBase}{if $config.mod_rewrite}{if $mItem.Path != ''}{$mItem.Path}.html{$mItem.Get_vars}{/if}{else}{if $mItem.Path != ''}?page={$mItem.Path}{$mItem.Get_vars|replace:'?':'&amp;'}{/if}{/if}">{if $account_info.Type == 'personal' && ($mItem.Key == 'add_listing' || $mItem.Controller == 'my_listings')}{$lang.escort_my_profile}{else}{$mItem.name}{/if}</a>
	...
	{else}
		<form {if $loginAttemptsLeft <= 0 && $config.security_login_attempt_user_module}onsubmit="return false;"{/if} class="account_login" action="{$rlBase}{if $config.mod_rewrite}{$pages.login}.html{else}?page={$pages.login}{/if}" method="post">
			...
			<span class="black_12">{$lang.forgot_pass}</span> <a title="{$lang.remind_pass}" class="brown_12" href="{$rlBase}{if $config.mod_rewrite}{$pages.remind}.html{else}?page={$pages.remind}{/if}">{$lang.remind_pass}</a><br />
			<span class="black_12">{$lang.new_here}</span> <a title="{$lang.create_account}" class="brown_12" href="{$rlBase}{if $config.mod_rewrite}{$pages.registration}.html{else}?page={$pages.registration}{/if}">{$lang.create_account}</a>
