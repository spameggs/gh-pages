...
<div id="fileupload">
	<form onsubmit="return false;" action="{$smarty.const.RL_LIBS_URL}upload/account.php" method="post" enctype="multipart/form-data">
		...
		<div style="padding-top: 10px;"><input type="button" class="start" value="{$lang.upload}" /></div>
	</form>
</div>
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
	<span {/literal}style="height: {$config.pg_upload_thumbnail_height}px;"{literal} class="template-upload fade item active">
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
	<span {/literal}style="height: {$config.pg_upload_thumbnail_height}px;"{literal} class="template-download fade item active">
