...
(function($){
	$.flModal = function(el, options){
		...
		base.loadContent = function(){
			...
			var dom = '<div class="inner"><div class="modal_content"></div><div class="close">'+lang['close']+'</div></div>';
			...
			/* clear source objects to avoid id overload */
...
/**
*
* grid bottom-layer action
*
**/
var flGridHover = function(){
	$('#listings div.item').mouseenter(function(){
		var self = this;
...
var bottom_layer_show = function(obj){
	$(obj).find('div.b-layer').attr('class', 'b-layer-active');
	$(obj).find('.nav').show();
}
var bottom_layer_hide = function(){
	$('#listings div.item div.b-layer-active').attr('class', 'b-layer');
	$('#listings div.item .nav').hide();
}
...
$(document).ready(function(){
	$('table.grid_navbar td.switcher>div').click(function(){
		...
		if ( type == 'list' )
		{
			gridActiveHeight = $('div#agencies>table.table').height();
			$('div#agencies>table.table').fadeOut('normal', function(){
				$('div#agencies>table.table').after('<div class="list"></div>');
				$('div#agencies>div.list').height(gridActiveHeight);
				transformToList(0);
			});
		}
		else
		{
			gridActiveHeight = $('div#agencies>div.list').height();
			$('div#agencies').css('min-height', gridActiveHeight+'px');
			gridLength = $('div#agencies>div.list>div.item').length;
			$('div#agencies>div.list').fadeOut('normal', function(){
				$('div#agencies>div.list').after('<table class="table"></table>');
...
var transformToList = function(index){
	var item = $('div#agencies>table.table>tbody>tr>td:not(.divider):eq('+index+')');
	...
	$(item).find('div.item').find('table>tbody>tr:first>td:last>table tr td:last').html($(item).find('div.item').find('table:first>tbody>tr:last>td').html());
	...
	$('div#agencies>div.list').append($(item).find('div.item'));
	$('div#agencies>div.list>div.item:last').fadeIn();
	var length = $('div#agencies>table.table>tbody>tr>td:not(.divider)').length;
	...
	else
	{
		...
		$('div#agencies>div.list').css('height', 'auto');
		$('div#agencies>table.table').remove();
	}
};

var gridAvailableRow = false;
var transformToTable = function(index){
	var item = $('div#agencies>div.list>div.item:first');
	$(item).hide();
	...
	if ( !gridAvailableRow )
	{
		$('div#agencies>table.table').append('<tr><td></td><td class="divider"></td><td></td></tr>');
		$('div#agencies>table.table>tbody>tr:last>td:first').html($(item));
		gridAvailableRow = true;
	}
	else
	{
		$('div#agencies>table.table>tbody>tr:last>td:last').html($(item));
		gridAvailableRow = false;
	}
	
	$('div#agencies>table.table div.item:last').fadeIn();
	...
	{
		gridInProgress = false;
		$('div#agencies>div.list').remove();
		$('div#agencies').attr('style', false);
	}
};
