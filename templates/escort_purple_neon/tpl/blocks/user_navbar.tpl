...
<div id="user_navbar">
	...
	<div class="center">
		{if $isLogin}
			<a title="{$lang.account_area}" class="account" href="javascript:void(0)"><span class="icon"></span>{$isLogin}<span></span></a>
			...
			<ul class="menu hide">
				{foreach from=$account_menu item='mItem'}
					{if $mItem.Key == 'my_messages' && !$config.messages_module}{else}
						<li>
							<a {if $page == $mItem.Path}class="active"{/if} title="{if $account_info.Type == 'personal' && ($mItem.Key == 'add_listing' || $mItem.Controller == 'my_listings')}{$lang.escort_my_profile}{else}{$mItem.title}{/if}" href="{if $mItem.Page_type == 'external'}{$mItem.Controller}{else}{$rlBase}{if $config.mod_rewrite}{if $mItem.Path != ''}{$mItem.Path}.html{$mItem.Get_vars}{/if}{else}{if $mItem.Path != ''}?page={$mItem.Path}{$mItem.Get_vars|replace:'?':'&'}{/if}{/if}{/if}">{if $account_info.Type == 'personal' && ($mItem.Key == 'add_listing' || $mItem.Controller == 'my_listings')}{$lang.escort_my_profile}{else}{$mItem.name}{/if}</a>
		...
		{else}
			<a title="{$lang.create_account}" class="registration" href="{$rlBase}{if $config.mod_rewrite}{$pages.registration}.html{else}?page={$pages.registration}{/if}"><span class="icon"></span>{$lang.registration}</a>
			<a title="{$lang.login}" class="login" href="javascript:void(0)"><span class="icon"></span>{$lang.login}</a>
		{/if}
