<!-- my packages tpl -->
{if $renew_id}
<div class="highlight"> {include file='blocks'|cat:$smarty.const.RL_DS|cat:'fieldset_header.tpl' id='plan_block' name=$lang.plan_details}
  <table class="plans">
    <tr class="no_hover">
      <td class="label"><table class="bg{if $pack_info.Limit > 0 && $pack_info.Using == 0 && $pack_info.Using != ''} o60{/if}">
          <tr>
            <td class="left" {if $pack_info.Color}style="background-color: #{$pack_info.Color};"{/if}></td>
            <td class="center" {if $pack_info.Color}style="background-color: #{$pack_info.Color};"{/if}><div class="price">{if $pack_info.Price > 0}
                {if $config.system_currency_position == 'before'}{$config.system_currency}{/if}
                {if $config.price_decimal == '.'}{$pack_info.Price}{else}{$pack_info.Price|replace:".":$config.price_decimal}{/if}
                {if $config.system_currency_position == 'after'}{$config.system_currency}{/if}
                {else}{$lang.free}{/if}</div>
              <div class="type">{assign var='l_type' value=$pack_info.Type|cat:'_plan_short'}{$lang.$l_type}</div></td>
            <td class="right"><div class="relative">
                <div {if $pack_info.Color}style="background-color: #{$pack_info.Color};"{/if}>
                  <div class="tile" style="background-color: #{if $plan.Color}{$plan.Color}{else}cccccc{/if};"></div>
                  <div class="bg"></div>
                </div>
              </div></td>
          </tr>
        </table></td>
      <td class="info"><table class="sTable">
          <tr>
            <td class="caption"><div>{$pack_info.name}</div></td>
            <td><ul class="features">
                ...
                {if $pack_info.Image || $pack_info.Image_unlim}
                <li class="pics" title="{$lang.images_number}">{if $pack_info.Image_unlim}{$lang.unlimited}{else}{$pack_info.Image}{/if}</li>
                {/if}
                {if $pack_info.Video || $pack_info.Video_unlim}
                <li class="video" title="{$lang.number_of_videos}">{if $pack_info.Video_unlim}{$lang.unlimited}{else}{$pack_info.Video}{/if}</li>
                {/if}
              </ul></td>
          </tr>
        </table>
  ...
  <form onsubmit="return flynax.isGatewaySelected();" name="payment" method="post" action="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/purchase.html{else}index.php?page={$pageInfo.Path}&amp;purchase{/if}">
    ...
    <table class="plans">
      ...
      <tr {if $plan.ID == $smarty.post.plan}class="active"{/if}> {assign var='item_disabled' value=false}
        ...
        <td class="label"><table class="bg{if $plan.Limit > 0 && $plan.Using == 0 && $plan.Using != ''} o60{/if}">
            <tr>
              ...
              <td class="center" {if $plan.Color}style="background-color: #{$plan.Color};"{/if}><div class="price">{if isset($plan.Listings_remains)}&rarr;{else}
                  ...
                  {if $config.system_currency_position == 'after'}{$config.system_currency}{/if}{else}{$lang.free}{/if}{/if}</div>
        ...
        <td class="info"><table class="sTable{if $plan.Limit > 0 && $plan.Using == 0 && $plan.Using != ''} o60{/if}">
            <tr>
              ...
              <td class="ralign"><div class="status" title="{$lang.package_purchased}">{$lang.already_purchased}</div></td>
              {/if} </tr>
          </table>
          <div class="desc">
...
<table class="list">
  ...
  <tr class="body">
    <td class="first" valign="top"><table>
        <tr>
          <td valign="top" class="price"><div>{if $package.Price > 0}
              {if $config.system_currency_position == 'before'}{$config.system_currency}{/if}
              {if $config.price_decimal == '.'}{$package.Price}{else}{$package.Price|replace:".":$config.price_decimal}{/if}
              {if $config.system_currency_position == 'after'}{$config.system_currency}{/if}{else}{$lang.free}{/if}</div></td>
          <td valign="top"><span class="dark_12"><b>{$package.name}
            </div></td>
          <td valign="top"><span class="dark_12"><b>{$package.name}</b></span>
    ...
    <td class="divider"></td>
    <td valign="top"><ul class="features">
      ...
      <ul class="package_info">
        ...
        <li> {$lang.standard}: <span {if empty($package.Standard_remains) && !empty($package.Standard_listings)}class="overdue"{/if}> {if empty($package.Standard_listings)} <b>{$lang.unlimited}</b> {else}
          ...
          {/if} </span> </li>
        <li> {$lang.featured}: <span {if empty($package.Featured_remains) && !empty($package.Featured_listings)}class="overdue"{/if}> {if empty($package.Featured_listings)} <b>{$lang.unlimited}</b> {else}
          {assign var='rRest' value=`$smarty.ldelim`rest`$smarty.rdelim`}
          {assign var='rAmount' value=`$smarty.ldelim`amount`$smarty.rdelim`}
          {$lang.rest_of_amount|replace:$rRest:$package.Featured_remains|replace:$rAmount:$package.Featured_listings}
          {/if} </span> </li>
        {else}
        <li> {if $package.Featured}
          {$lang.featured}: 
          {else}
          {$lang.standard}: 
          {/if} <span {if empty($package.Listings_remains) && !empty($package.Listing_number)}class="overdue"{/if}> {if empty($package.Listing_number)} <b>{$lang.unlimited}</b> {else}
          {assign var='rRest' value=`$smarty.ldelim`rest`$smarty.rdelim`}
          {assign var='rAmount' value=`$smarty.ldelim`amount`$smarty.rdelim`}
    ...
    <td class="divider"></td>
    <td valign="top"><span class="{$package.Exp_status}"> {if $package.Exp_date == 'unlimited'}
      {$lang.unlimited}
      {else}
      {$package.Exp_date|date_format:$smarty.const.RL_DATE_FORMAT}
      {/if} </span>
      <div style="text-align: center;padding: 10px 0 0;"> <a title="{$lang.renew}" href="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}.html?renew={$package.ID}{else}index.php?page={$pageInfo.Path}&amp;renew={$package.ID}{/if}" class="renew"><span></span></a> </div></td>
  </tr>
  {/foreach}
</table>
<div style="padding: 15px 0 0 0; text-align: right;"> <a href="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/purchase.html{else}index.php?page={$pageInfo.Path}&amp;purchase{/if}" class="button">{$lang.purchase_new_package}</a> </div>
</div>
{else}
        ...
        {/if}
        {assign var='replace' value='<a href="'|cat:$link|cat:'" class="static">$1</a>'} <span class="info">{$lang.no_packages_available|regex_replace:'/\[(.*)\]/':$replace}</span> {/if}
        {/if} 
<!-- my packages tpl end -->