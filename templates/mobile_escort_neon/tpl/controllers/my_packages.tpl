      ...
      <td class="label"><table class="bg{if $pack_info.Limit > 0 && $pack_info.Using == 0 && $pack_info.Using != ''} o60{/if}">
          <tr>
            ...
            <td class="right"><div class="relative">
                <div {if $pack_info.Color}style="background-color: #{$pack_info.Color};"{/if}> {if $pack_info.Color}
                  <div class="tile" style="background-color: #{$pack_info.Color};"></div>
                  {/if}
                  <div class="bg"></div>
                </div>
              </div></td>
  ...
  <form onsubmit="return flynax.isGatewaySelected();" name="payment" method="post" action="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}/purchase.html{else}index.php?page={$pageInfo.Path}&amp;purchase{/if}">
    ...
    <table class="plans">
      ...
      <tr {if $plan.ID == $smarty.post.plan}class="active"{/if}> {assign var='item_disabled' value=false}
        ...
        <td class="info"><div class="title">{$plan.name} {if $plan.ID|in_array:$used_plans_id}({$lang.already_purchased}){/if}</div>
          <ul class="features">
...
<div class="highlight">
  <table class="list">
    ...
    <tr class="body">
      <td class="first" valign="top"><div class="price">{if $package.Price > 0}
          {if $config.system_currency_position == 'before'}{$config.system_currency}{/if}
      ...
      <td valign="top"><ul class="features">
        ...
        <div class="package_date">
          ...
          <span class="{$package.Exp_status}"> {if $package.Exp_date == 'unlimited'}
          {$lang.unlimited}
          {else}
          {$package.Exp_date|date_format:$smarty.const.RL_DATE_FORMAT}
          {/if} </span>
          <div style="text-align: center;padding: 10px 0 0;"> <a title="{$lang.renew}" href="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}.html?renew={$package.ID}{else}index.php?page={$pageInfo.Path}&amp;renew={$package.ID}{/if}" class="renew"><span></span></a> </div>
        </div></td>
    </tr>
    {/foreach}
  </table>
