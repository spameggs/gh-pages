		...
		{if !empty($all_news)}
			{foreach from=$all_news item='news'}
				<div style="padding: 0 0 20px">
					<span class="news-date">{$news.Date|date_format:$smarty.const.RL_DATE_FORMAT}</span>
					<a class="news-caption" title="{$news.title}" href="{$rlBase}{if $config.mod_rewrite}{$pages.news}/{$news.Path}.html{else}?page={$pages.news}&amp;id={$news.ID}{/if}">{$news.title}</a>
