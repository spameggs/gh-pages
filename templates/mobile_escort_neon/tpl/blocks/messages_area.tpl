<!-- messages area DOM -->
	...
	<tr class="body{if $message.Hide} removed{/if}{if $message.To == $account_info.ID} hlight{/if}">
		<td style="width: 60px;padding-left: 0;padding-right: 0;" valign="top" align="center">
			...
			{if $message.To != $account_info.ID}
				{if $account_info.Photo}
					<img alt="" style="width: 36px;" class="photo" src="{$smarty.const.RL_URL_HOME}files/{$account_info.Photo}" />
				{else}
					<img alt="" class="avatar40" src="{$rlTplBase}img/blank.gif" />
				{/if}
			{else}
				{if $contact.Photo}
					<img alt="" style="width: 36px;" class="photo" src="{$smarty.const.RL_URL_HOME}files/{$contact.Photo}" />
				{else}
					<img alt="" class="avatar40" src="{$rlTplBase}img/blank.gif" />
				{/if}
			{/if}
