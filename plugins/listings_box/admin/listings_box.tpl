...
{assign var='sPost' value=$smarty.post}
	...
	{include file='blocks'|cat:$smarty.const.RL_DS|cat:'m_block_start.tpl'}
		<form action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;block={$smarty.get.block}{/if}" method="post" enctype="multipart/form-data">
			...
			<tr id="columns" {if $display_mode_yes}class="hide"{/if} >
				<td class="name">{$lang.listings_box_columns}</td>
				<td class="field">
					<input type="text" class="numeric" style="width: 60px; text-align: center;" name="columns" value="{if $sPost.columns}{$sPost.columns}{else}0{/if}"  maxlength="2" />
				</td>
			</tr>
			<tr>
	...
	{/if}
	{literal}
		$("#display_mode input").click(function() {
			if($(this).val() == 'grid')
			{
