...
{if $config.browse_map_module}
	...
	{if $pageInfo.Key == 'listings'}
		{foreach from=$listing_types item='tab' key='lt_key' name='tabsF'}
			{if $requested_type == $lt_key}
				{assign var='bmFirstKey' value=$lt_key|replace:'_':''}
			{/if}
			<div class="bm_maps{if $requested_type != $lt_key} hide{/if}" {if !$block.Tpl}style="padding: 0 0 10px;"{/if}>
				<div class="{if !$block.Tpl}highlight highlight_loading{/if}" id="bm_map_{$lt_key|replace:'_':''}" style="{if $config.browse_map_width}width: {$config.browse_map_width}px;{/if}height: {if $config.browse_map_height}{$config.browse_map_height}{else}250{/if}px;"></div>
			</div>
		{/foreach}
	{else}
		<div {if !$block.Tpl}style="padding: 0 0 10px;"{/if}>
			<div class="{if !$block.Tpl}highlight highlight_loading{/if}" id="bm_map" style="{if $config.browse_map_width}width: {$config.browse_map_width}px;{/if}height: {if $config.browse_map_height}{$config.browse_map_height}{else}250{/if}px;"></div>
		</div>
	{/if}
