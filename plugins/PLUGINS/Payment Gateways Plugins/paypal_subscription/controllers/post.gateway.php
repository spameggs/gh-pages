...
if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
    ...
    if ($config['paypal_subscription_test_mode']) {
        $testMode = "sandbox.";
    }
    $host = "www.{$testMode}paypal.com";
    ...
    if ($fp) {
        ...
        if (strcmp($arr[1], 'VERIFIED') != 0) {
            $rlDebug->logger("PayPal Subscripion: Exit since PayPal returned '{$arr[1]}' as status");
            exit;
        }
        if (!in_array(trim(strtolower($payment_status)), array(
            ...
            $rlDebug->logger("PayPal Subscripion: Exit since payment status is not Completed or Pending");
            exit;
        }
        ...
        if (strcmp($crypted_sum, crypt(sprintf("%.2f", $total), $config['paypal_secret_word'])) != 0) {
            $rlDebug->logger("PayPal Subscripion: Exit since crypted sum is invalid");
            exit;
        }
        if ($callback_plugin) {
        ...
        $$callback_class->$callback_method($item_id, $plan_id, $account_id, $txn_id, 'PayPal Subscription', $total);
        $rlDb->query("UPDATE `" . RL_DBPREFIX . "listings` SET `Subscription_data` = 'paypal_subscription', `Cron_notified` = '1' WHERE `ID` = '{$item_id}' LIMIT 1");
    }
}