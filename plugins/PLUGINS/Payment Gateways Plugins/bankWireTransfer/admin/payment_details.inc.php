...
if (isset($_GET['action'])) {
    $bcAStep[0] = array(
        'name' => $lang['bwt_payment_wire_transfer'],
        'Controller' => 'bank_wire_transfer',
        'Vars' => 'module=payment_details'
    );
    $bcAStep[1] = array(
        'name' => $_GET['action'] == 'add' ? $lang['bwt_add_item'] : $lang['bwt_edit_item']
    );
    if ($_GET['action'] == 'add' || $_GET['action'] == 'edit') {
        $reefless->loadClass('Account');
        if (isset($_GET['item'])) {
            $id             = (int) $_GET['item'];
            $payment_detail = $rlDb->fetch('*', array(
                'ID' => $id
            ), null, null, 'bwt_payment_details', 'row');
        ...
        if ($_GET['action'] == 'edit' && !$_POST['fromPost']) {
            ...
            $names        = $rlDb->fetch(array(
                ...
                'Key' => 'payment_details+name+' . $payment_detail['Key']
            ...
            $descriptions = $rlDb->fetch(array(
                ...
                'Key' => 'payment_details+des+' . $payment_detail['Key']
            ), "AND `Status` <> 'trash'", null, 'lang_keys');
