...
class Smarty_Compiler extends Smarty
{
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        if (!$_SESSION['reCaptcha_system']) {
            preg_match("/'captcha_id'\s*\=\>\s*'([^']*)'/", $compiled_content, $vs_matches);
            preg_match("/(captcha\.tpl)/", $compiled_content, $captcha_matches);
            if ($captcha_matches[1]) {
                $compiled_content             = preg_replace("/('captcha\.tpl')/", "RL_PLUGINS .'reCaptcha'. RL_DS .'reCaptcha.tpl'", $compiled_content, 1);
                $_SESSION['reCaptcha_system'] = true;
                if ($vs_matches[1]) {
                    $compiled_content .= ' <?php $_SESSION["reCaptcha_items"][] = "' . $vs_matches[1] . '"; ?> ';
                }
            }
        }
        return true;
