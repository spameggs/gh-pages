...
$date_format = $rlDb->fetch(array(
    'Date_format'
), array(
    'Code' => $config['lang']
), null, 1, 'languages', 'row');
...
if ($listings) {
    $in_days = $config['cron_expired_listing_days'];
...
if (empty($listings)) {
    ...
    foreach ($listings as $key => $listing) {
        ...
        if ($listing['Pay_date'] && ($listing['Pay_date'] + ($listing['Listing_period'] * 86400) <= time()) && $listing['Listing_period'] > 0) {
            ...
            $copy_listing_expired_email['body']    = str_replace(array(
                '{username}',
                '{renew_link}',
                '{details_link}'
            ), array(
                ...
                $update_link,
                $details_link
            ), $copy_listing_expired_email['body']);
            $copy_listing_expired_email['subject'] = str_replace(array(
                '{username}',
                '{renew_link}',
                '{details_link}'
            ), array(
                ...
                $update_link,
                $details_link
            ), $copy_listing_expired_email['subject']);
        ...
        if ($listing['Feature_period'] && $listing['Featured_date'] && ($listing['Featured_date'] + ($listing['Feature_period'] * 86400) <= time()) && $listing['Feature_period'] > 0) {
            ...
            $copy_status_expired_email['body']    = str_replace(array(
                '{username}',
                '{renew_link}',
                '{details_link}'
            ), array(
                ...
                $update_link,
                $details_link
            ), $copy_status_expired_email['body']);
            $copy_status_expired_email['subject'] = str_replace(array(
                '{username}',
                '{renew_link}',
                '{details_link}'
            ), array(
                ...
                $update_link,
                $details_link
            ), $copy_status_expired_email['subject']);
        ...
        if ($listing['Pay_date'] && ($listing['Cron_notified'] == '0') && ($listing['Pay_date'] + (($listing['Listing_period'] * 86400) - ($expire_days * 86400)) <= time()) && $listing['Listing_period'] > 0) {
            ...
            $copy_listing_pre_expired['body']    = str_replace(array(
                '{username}',
                '{days}',
                '{expire_date}',
                '{renew_link}',
                ...
                $update_link,
                $details_link
            ), $copy_listing_pre_expired['body']);
            $copy_listing_pre_expired['subject'] = str_replace(array(
                '{username}',
                '{days}',
                '{expire_date}',
                '{renew_link}',
                ...
                $update_link,
                $details_link
            ), $copy_listing_pre_expired['subject']);
        ...
        if ($listing['Feature_period'] && $listing['Featured_date'] && ($listing['Cron_featured'] == '0') && ($listing['Featured_date'] + (($listing['Feature_period'] * 86400) - ($expire_days * 86400)) <= time()) && $listing['Feature_period'] > 0) {
            ...
            $copy_status_pre_expired['body']    = str_replace(array(
                '{username}',
                '{days}',
                '{expire_date}',
                '{renew_link}',
                ...
                $update_link,
                $details_link
            ), $copy_status_pre_expired['body']);
            $copy_status_pre_expired['subject'] = str_replace(array(
                '{username}',
                '{days}',
                '{expire_date}',
                '{renew_link}',
                ...
                $update_link,
                $details_link
            ), $copy_status_pre_expired['subject']);
    ...
    foreach ($searches as $key => $search) {
        ...
        if (!empty($matches)) {
            ...
            if ($allow_send) {
                ...
                $copy_notify_email['body']         = str_replace(array(
                    '{username}',
                    '{count}',
                    '{links}'
                ), array(
                ...
                $copy_notify_email['subject']      = str_replace(array(
                    '{username}',
                    '{count}'
                ), array(
                    $account_info['Full_name'],
...
if ($listings) {
    ...
    foreach ($listings as $key => $listing) {
        if ($listing['Cron_notified']) {
            ...
            $copy_incomplete_listing['body'] = str_replace(array(
                '{username}',
                '{listing_title}',
                '{number_days}',
                '{complete_link}',
