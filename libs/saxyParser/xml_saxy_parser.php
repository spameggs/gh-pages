            ...
            switch ($this->state) {
                ...
                case SAXY_STATE_PROLOG_COMMENT:
                    if ($currentChar == '>') {
                        ...
                        if ((($xmlText{($i + 1)} == '-') && ($xmlText{($i + 2)} == '>')) || ($xmlText{($i + 1)} == '>') || (($xmlText{($i - 1)} == '-') && ($xmlText{($i - 2)} == '!'))) {
                        } else {
                            $this->charContainer .= $currentChar;
                        }
                    } else {
                ...
                case SAXY_STATE_PARSING:
                    switch ($currentChar) {
                        case '<':
                            ...
                            break;
                        case '-':
                            if (($xmlText{($i - 1)} == '-') && ($xmlText{($i - 2)} == '!') && ($xmlText{($i - 3)} == '<')) {
                    ...
                    switch ($currentChar) {
                        case '>':
                            if (($xmlText{($i - 1)} == '-') && ($xmlText{($i - 2)} == '-')) {
                                $this->fireCommentEvent(substr($this->charContainer, 0, (strlen($this->charContainer) - 2)));
                                $this->charContainer = '';
                                $this->state         = SAXY_STATE_PARSING;
                            } else {
                                $this->charContainer .= $currentChar;
